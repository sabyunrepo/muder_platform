...
When:
- 사용자가 비단순 기능 추가, 버그 수정, UI/UX 개선, 백엔드 오류 진단, PR/merge까지 이어지는 작업을 요청할 때
...
7. PR/merge가 요청된 경우 `mmp-pr-lifecycle` skill과 `scripts/pr-create-guard.sh` 경로를 사용한다.

Done when:
- 원인, 수정 범위, 검증 결과, 남은 위험, PR/merge 상태가 구분되어 보고된다.
- 사용자가 마지막 검토를 요청한 작업은 PR 생성 또는 merge 전에 멈춰 검토 지점을 알려준다.

Avoid:
- 원인 확인 없이 추측으로 구현하지 않는다.
- OOO run이나 sub-agent 결과가 요청 범위를 크게 벗어나면 그대로 적용하지 말고 중단 후 좁은 패치로 회수한다.
- local-ci 실패를 “테스트 실패”로 단정하지 않고, dirty diff, generated drift, 의존성 설치 문제, 실제 코드 실패를 분리해 보고한다.
...
- 플랜, 작업 지시, sub-agent handoff, 실행 원장을 만들 때는 Markdown todo 체크박스(`- [ ]`)로 항목을 작성한다. 진행 중 실제 완료된 항목은 같은 세션 안에서 `- [x]`로 갱신해 사용자가 어디까지 끝났는지 바로 볼 수 있게 한다.
- cmux 브라우저가 사용 중이거나 사용자가 "cmux 브라우저"로 확인하라고 한 로컬 웹 검증은 `cmux-browse` skill/도구를 우선 사용한다. Playwright MCP, in-app Browser, macOS `open`은 cmux 도구가 없거나 실패한 뒤에만 fallback으로 쓰고, fallback 이유를 보고한다.
...
- 사용자가 OOO, agentic harness, 독립 리뷰/검증, issue-to-PR chain, 또는 "본인이 리뷰/검증 금지"를 명시했을 때
...
6. 공유 계약(API DTO, frontend adapter/ViewModel mapping, migration, generated client, route contract)은 `mmp-contract-integrator` 또는 메인 Codex의 순차 통합 소유로 둔다. 여러 구현 에이전트가 같은 계약 파일을 동시에 수정하지 않는다.
7. 리뷰 sub-agent 결과는 사용자에게 raw output으로 붙이지 말고 `발견 / 수행 / 판단 / 미해결` 4섹션으로 압축한다.
...
4. backend handler/service, frontend adapter/hook, 저장/삭제 guard는 성공 경로뿐 아니라 validation, not found, ownership, conflict, delete-blocked 같은 실패 경로도 focused test에 포함한다.
5. 로컬 focused test와 필요 시 `scripts/mmp-local-ci.sh quick`으로 변경 위험을 검증한다.

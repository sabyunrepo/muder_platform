...
## 3차 findings (PR #51 + test selector 실험, 2026-04-16)

### H7 발견: `CreateRoomRequest.MaxPlayers` 서버 필수, 프론트 미전송

test selector를 `state: "attached"`로 바꾸자 Playwright가 실제 테마 옵션을 발견하고 createRoom이 생성 버튼 클릭까지 진행. 하지만 서버 로그에:

```
WRN Key: 'CreateRoomRequest.MaxPlayers' Error:Field validation for 'MaxPlayers' failed on the 'required' tag code=VALIDATION_ERROR
WRN request completed bytes=200 method=POST path=/api/v1/rooms status=400
```

`POST /api/v1/rooms`가 400. 프론트 `CreateRoomModal.tsx`의 create mutation payload가 `MaxPlayers`를 안 보냄 (또는 서버 CreateRoomRequest 구조가 변경됐지만 FE 반영 안 됨).

### Phase 18.6 스코프 판정

H7는 H6와 동일한 성격(FE/BE 계약 drift)이지만 수정 범위는 더 넓음(RoomPage 페이로드까지). Phase 18.6 "E2E Recovery" 스코프(login timeout + theme seed)를 넘어감.

조치:
- test selector 변경 **revert** (createRoom은 다시 NO_THEMES로 graceful skip)
- Phase 18.6은 login 복구 + H6 schema 정렬로 종료
- H7(MaxPlayers)과 나머지 createRoom/startGame contract 점검은 **Phase 18.7**로 이관

### Phase 18.6 최종 상태

| 항목 | 상태 |
|------|------|
| login beforeEach | ✅ 통과 (ws-client build fix) |
| lobby 렌더링 | ✅ 안정 (H6 schema 정렬 + 방어 렌더) |
| Theme seed CI step | ✅ 재활성 |
| createRoom 이후 scenarios | ⏭ NO_THEMES graceful skip (H7 이관) |
| E2E 4 pass / 11 skipped / 0 fail | ✅ |

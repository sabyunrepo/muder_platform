...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  MEDIA_REFERENCE_CLEANUP_FAILED:
    '미디어 연결 해제 중 오류가 발생했습니다. 새로고침 후 다시 시도해주세요.',
  MEDIA_STORAGE_DELETE_FAILED:
    '파일 저장소에서 미디어를 삭제하지 못했습니다. 잠시 후 다시 시도해주세요.',
  MEDIA_CONFIG_REFERENCE_INVALID:
    '테마 설정에 잘못된 미디어 연결 정보가 있어 자동 정리할 수 없습니다. 관리자에게 문의해주세요.',
...
export function getUserMessage(error: ApiError): string {
  ...
  const template = (error.code && ERROR_MESSAGES[error.code]) ?? FALLBACK_MESSAGE;

  if (error.params) {
    return template.replace(/\{(\w+)\}/g, (_, key: string) =>
      String(error.params?.[key] ?? `{${key}}`)
    );
  }

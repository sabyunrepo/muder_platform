export const EDITOR_DESIGN_SCOPE_CLASS = "mmp-editor-design-scope";

export const editorDesignClassNames = {
  surface: "mmp-editor-surface",
  panel: "mmp-editor-panel",
  topBar: "mmp-editor-top-bar",
  tabBar: "mmp-editor-tab-bar",
  tab: "mmp-editor-tab",
  tabActive: "mmp-editor-tab-active",
  listItem: "mmp-editor-list-item",
  listItemActive: "mmp-editor-list-item-active",
  subtlePanel: "mmp-editor-subtle-panel",
  errorPanel: "mmp-editor-error-panel",
  input: "mmp-editor-input",
  dangerAction: "mmp-editor-danger-action",
  iconButton: "mmp-editor-icon-button",
  sectionHeader: "mmp-editor-section-header",
  mutedText: "mmp-editor-muted-text",
  primaryAction: "mmp-editor-primary-action",
  secondaryAction: "mmp-editor-secondary-action",
  tag: "mmp-editor-tag",
} as const;

export const editorNotionTokenSource = "DESIGN.md";

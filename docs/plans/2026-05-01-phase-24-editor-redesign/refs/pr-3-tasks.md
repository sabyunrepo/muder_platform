...
- [x] `CharacterRoleSheetSection` 추가
  - ~~`useEditorContent(themeId, role_sheet:<id>)`~~ → `useCharacterRoleSheet(characterId)`
  - ~~`useUpsertContent(themeId, role_sheet:<id>)`~~ → `useUpsertCharacterRoleSheet(characterId)`
  - blur/save 버튼으로 저장
...
- [x] PR-3B: voting candidate policy 작업 착수
- [x] PR-3C: typed role sheet API 작업 착수
- [x] PR-3D: DOCUMENT/PDF 업로드 + viewer 작업 착수
- [x] PR-3E: image role sheet viewer 작업 착수

...
func (q *Queries) ApproveSettlement(ctx context.Context, arg ApproveSettlementParams) (Settlement, error) {
	row := q.db.QueryRow(ctx, approveSettlement, arg.ID, arg.ApprovedBy)
	var i Settlement
	err := row.Scan(
		&i.ID,
		&i.CreatorID,
...
func (q *Queries) CollectUnsettledEarnings(ctx context.Context) ([]CollectUnsettledEarningsRow, error) {
	...
	defer rows.Close()
	items := []CollectUnsettledEarningsRow{}
	for rows.Next() {
...
func (q *Queries) CountSettlementsByStatus(ctx context.Context, dollar_1 string) (int64, error) {
	row := q.db.QueryRow(ctx, countSettlementsByStatus, dollar_1)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
SELECT id, creator_id, theme_id, purchase_id, total_coins, creator_share_coins, platform_share_coins, settled, settlement_id, created_at FROM creator_earnings WHERE purchase_id = $1
`
...
SELECT id, creator_id, period_start, period_end, total_coins, total_krw, tax_type, tax_rate, tax_amount, net_amount, status, approved_by, approved_at, paid_out_at, created_at, updated_at FROM settlements WHERE id = $1
`
...
type GetThemeDailyStatsParams struct {
	ThemeID     uuid.UUID `json:"theme_id"`
	CreatorID   uuid.UUID `json:"creator_id"`
...
func (q *Queries) GetThemeDailyStats(ctx context.Context, arg GetThemeDailyStatsParams) ([]GetThemeDailyStatsRow, error) {
	rows, err := q.db.Query(ctx, getThemeDailyStats,
		arg.ThemeID,
		arg.CreatorID,
		arg.CreatedAt,
		arg.CreatedAt_2,
...
type ListEarningsByCreatorParams struct {
	CreatorID uuid.UUID `json:"creator_id"`
...
func (q *Queries) ListEarningsByCreator(ctx context.Context, arg ListEarningsByCreatorParams) ([]ListEarningsByCreatorRow, error) {
	rows, err := q.db.Query(ctx, listEarningsByCreator, arg.CreatorID, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListEarningsByCreatorRow{}
	for rows.Next() {
...
SELECT id, creator_id, period_start, period_end, total_coins, total_krw, tax_type, tax_rate, tax_amount, net_amount, status, approved_by, approved_at, paid_out_at, created_at, updated_at FROM settlements
WHERE creator_id = $1
ORDER BY created_at DESC
LIMIT $2 OFFSET $3
`

type ListSettlementsByCreatorParams struct {
	CreatorID uuid.UUID `json:"creator_id"`
	Limit     int32     `json:"limit"`
	Offset    int32     `json:"offset"`
}

func (q *Queries) ListSettlementsByCreator(ctx context.Context, arg ListSettlementsByCreatorParams) ([]Settlement, error) {
	rows, err := q.db.Query(ctx, listSettlementsByCreator, arg.CreatorID, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
type ListSettlementsByStatusParams struct {
...
func (q *Queries) ListSettlementsByStatus(ctx context.Context, arg ListSettlementsByStatusParams) ([]ListSettlementsByStatusRow, error) {
	rows, err := q.db.Query(ctx, listSettlementsByStatus, arg.Column1, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListSettlementsByStatusRow{}
	for rows.Next() {
...
type SettleEarningsParams struct {
	CreatorID    uuid.UUID   `json:"creator_id"`
	SettlementID pgtype.UUID `json:"settlement_id"`
}

func (q *Queries) SettleEarnings(ctx context.Context, arg SettleEarningsParams) error {
	_, err := q.db.Exec(ctx, settleEarnings, arg.CreatorID, arg.SettlementID)
	return err
}
...
func (q *Queries) UnsettleEarningsBySettlement(ctx context.Context, settlementID pgtype.UUID) error {

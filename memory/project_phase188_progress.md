...
| 2 | PR-1 security | validation 에러 detail에서 theme range 마스킹 (oracle 방어) | minor | ✅ hotfix #4 |
| 3 | PR-1 code | ListWaitingRooms `mapRoomResponse` DRY 정리 | minor | ✅ hotfix #4 |
...
| 6 | PR-3 LOW | detective fallback 향후 차등 공개 도입 시 drift 가능 | low | ✅ hotfix #4 (drift monitor 주석) |
| 7 | PR-4 docs | `refs/pr-4-...md` spec 문서 stale (POST/DELETE → GET/PUT) 반영 | low | ✅ hotfix #4 |
| 8 | PR-4 LOW | PUT invalid payload 분기 vitest 추가 | low | ✅ hotfix #4 |
| 9 | PR-4 LOW | fitView 애니메이션 중 click flaky 모니터링 | low | ✅ hotfix #4 (monitor 주석) |

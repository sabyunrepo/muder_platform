...
// OnEvent is called for every published event whose Type matches the subscription.
...
// Publish dispatches evt to all registered subscribers for evt.Type and
...
// Publish dispatches evt to all subscribers registered for evt.Type.
...
func (b *TypedEventBus) callSafe(ctx context.Context, sub Subscriber, evt GameEvent) (retErr error) {
	defer func() {
		if r := recover(); r != nil {
			...
			retErr = &panicError{topic: evt.Type, value: r}
		}
	}()

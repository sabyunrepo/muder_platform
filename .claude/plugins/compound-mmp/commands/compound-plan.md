...
> **superpowers 가용성 가정** (sister 카논 align — `review-mmp/SKILL.md` § OMC fallback 패턴 대칭): 본 repo는 `~/.claude/plugins/superpowers/`를 사용자 환경 카논으로 간주. compound-mmp `scripts/install.sh`(PR #160)의 정상 흐름에 포함된다. `superpowers:brainstorming`/`superpowers:writing-plans` 미가용 시 메인 컨텍스트가 `general-purpose` subagent + 본 SKILL의 brainstorm/write-plans 체크리스트로 대체 (review-mmp/SKILL.md fallback 표 대칭). **OMC 폴백은 review 단계 카논** (`review-mmp/SKILL.md` § "OMC fallback 매핑")이며 plan 단계에는 적용하지 않는다 — OMC가 OMC의 폴백이 될 수 없다는 단일 source 원칙.
...
## 검증 (test-compound-plan-dry-run.sh, 38 case)

helper 직접 실행 → JSON contract 검증. 입력 화이트리스트 (8 case) / 정상 입력 (4 case) / JSON 구조 (5 case) / 단계 매핑 (7 case) / 토큰 치환 (6 case) / `--from` 옵션 (5 case, HIGH-T1 가드 3 case 포함) / 자동 PR 금지 (1 case) / mandatory_slots 메타 (1 case) / 환경 변수 (2 case).

CI: `.github/workflows/ci-hooks.yml`이 ubuntu+macOS에서 fixture 실행 (`compound-review-dry-run.sh` sister fixture와 같은 카논).

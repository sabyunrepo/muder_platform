...
| W3 | PR-8 Module Cache Isolation + H-2 focus-visible | ✅ 완료 — PR-8 #137 `d3e6934` (docs-only) · H-2 #138 `691b86a` (axe-core spec 신규 + @axe-core/playwright 추가) |
| W4 | PR-9 WS Auth Protocol / PR-10 Runtime Payload Validation | ready — L+L 규모, 착수 대기 |
...
## W3 Cleanup — PR-8 + H-2 (2026-04-22)

### PR-8 Module Cache Isolation — 선제 완료 검증 (#137 `d3e6934`)
- **패턴**: PR-3 / PR-1 / H-1 와 동일 audit-snapshot drift. Phase 19 main implementation 의 #91 `34e952f` (2026-04-18) 로 이미 squash-merged.
- **검증 (file:line)**:
  - Task 1 factory key: `moduleStoreFactory.ts:45-47` `makeCacheKey(sessionId, moduleId)` + docstring "F-react-6"
  - Task 2 resetGame cleanup: `gameSessionStore.ts:89-95` + `moduleStoreCleanup.ts:29-38` prefix 매칭
  - Task 3 DEV warn: `moduleStoreFactory.ts:76-83` `!sessionId` 분기 + test 3건
  - Task 4 세션 전환 E2E: **Phase 21 orphan 이월** (Playwright Zustand inspection 비용 대비 효과 낮음). 대체 커버리지 39 unit (`moduleStoreFactory.test.ts`) + 10 integration (`useGameSync.test.ts`)
- **본 PR 실질 변경**: 2 파일 · 코드 0 · 문서 (+13/-9). checklist PR-8 섹션 "선제 완료" 로 전환 + W3 실작업량은 H-2 라는 명시.

### H-2 focus-visible — axe-core a11y smoke (#138 `691b86a`)
- **audit drift**: 57건(audit 시점) → 67건/41 파일 (현재, 신규 UI 증가분). Explore line-level audit 결과 **미병기 0건**. 67/67 전수 `focus-visible:outline-none + focus-visible:ring-2 + focus-visible:ring-amber-500` 패턴 + 다크모드 `ring-offset-slate-900`.
- **Task 1-2 선제 완료** · **Task 3 실작업**:
  - `apps/web/e2e/a11y-focus.spec.ts` 신규 60 LOC (2 tests: `/login` AxeBuilder WCAG 2.1 A/AA critical/serious 0건 + email/password/submit Tab 순회 focus indicator 실렌더 검증)
  - `@axe-core/playwright ^4.11.2` devDep 추가
- **검증**: spec Playwright list 확인 (rtk 토큰킬러가 출력을 변조해 raw 는 확인 불가, CI 본실행에 위임). 기존 `front-pages.spec.ts` 패턴 답습 (백엔드 독립 `/login` 라우트).

### W3 회고
- 2 PR 모두 "audit snapshot vs 현재 상태 drift" 패턴으로 확정. 실작업 LOC = 60 줄 (spec 1 개), 나머지는 전부 문서 갱신.
- W4 (PR-9/PR-10) 은 L+L 규모 — 설계부터 새로 브레인스토밍 필요. `/clear` + `/plan-resume` 재개 권장.
...
## Phase 20 Backlog (세션 회고 리뷰 도출 — 2026-04-21)

W1+W2 완료 후 critic + code-reviewer 병렬 회고 리뷰에서 도출. Phase 20 W0 (첫 주) 소화 대상:

- **B-1 Coverage threshold 재측정 + 상향** — PR-5c infra 복구로 Go baseline 이 43.9% 대비 올랐을 것. 재측정 후 threshold 를 43% 이상으로 상향. Phase 20 target 55% 로 향하는 단계 상향 로드맵 확정. 작업: `go test -coverprofile` → `go tool cover -func` → `.github/workflows/ci.yml` threshold 갱신.

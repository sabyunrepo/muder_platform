...
const { useBalanceMock, purchaseMutateMock, usePurchaseThemeMock, navigateMock, toastSuccess } =
  vi.hoisted(() => ({
...
describe('PurchaseThemeModal', () => {
  ...
  it('요약과 테마 정보 표면에 고정 slate dark class를 사용하지 않는다', () => {
    useBalanceMock.mockReturnValue({
      data: { base_coins: 800, bonus_coins: 200, total_coins: 1000 },
    });
    ...
    for (const className of forbiddenClasses) {
      expect(baseElement.innerHTML).not.toContain(className);
    }
  });
  ...
  it('잔액 부족 시 경고와 충전 링크를 표시한다', () => {
    ...
    expect(screen.getByText('코인이 부족합니다. 충전 후 다시 시도해주세요.')).toBeDefined();
  ...
  it('PURCHASE_SELF_THEME 에러 메시지를 표시한다', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('자신이 만든 테마는 구매할 수 없습니다.')).toBeDefined();
    });
  });
});

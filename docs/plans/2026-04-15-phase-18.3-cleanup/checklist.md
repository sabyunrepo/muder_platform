...
- [x] PR merged to main (PR #40 open)

### PR-1: CI infra debt
- [x] Task 1 — CI-1 `config.TestLoad_Defaults`: `os.Unsetenv` 으로 env 격리 + t.Setenv 로 명시 세팅, main CI 녹색
- [x] Task 2 — CI-2 golangci-lint Go 1.25 호환: latest tag (혹은 supported 버전) 로 업데이트, Makefile/CI 스크립트 반영
- [x] Task 3 — CI-3 ESLint 9 flat config: `eslint.config.js` 작성 + 기존 규칙 이식, `pnpm lint` 통과
- [x] Task 4 — GH Actions: golangci-lint + eslint job 명시적 추가 (없으면)
- [x] Task 5 — README/CONTRIBUTING 업데이트 (설치 명령어, 버전 명시)

**PR-1 gate**:
- [x] `golangci-lint run` pass 로컬
- [x] `pnpm lint` pass
- [x] `go test -race ./internal/config/...` pass (env 독립)
- [x] PR merged to main
...
- [x] Task 1 — L-3 `recentLeftAt` O(N) 선형 스캔: per-session sub-map 으로 전환 (`map[sessionID]map[playerID]time.Time`)
- [x] Task 2 — L-4 snapshotKey 네임스페이스 prefix `mmp:session:` — 마이그레이션 경로 정의 (24h 후 기존 키 자연소멸)
- [x] Task 3 — L-5 Hub.Stop 동시 writer 경합: 클라이언트 map 클리어 전에 stopping 플래그 설정 후 broadcast 루프가 플래그 체크
- [x] Task 4 — 회귀 테스트

### PR-3: E2E stubbed-backend CI job
- [x] Task 1 — L-8 Docker compose 서비스(Postgres+Redis+server stub) 를 CI 에서 기동
- [x] Task 2 — `PLAYWRIGHT_BACKEND=1` 자동 세팅 → skip guard 대부분 동작, 핵심 3 시나리오 필수 실행
- [x] Task 3 — nightly 뿐 아니라 PR 기본 CI job 에 포함 (optional → required)
- [x] Task 4 — 시나리오: 방 생성→시작→페이즈 진행, 재접속 복원, redaction 확인
...
- [x] Task 5 — `/plan-finish` (사용자가 직접 실행)

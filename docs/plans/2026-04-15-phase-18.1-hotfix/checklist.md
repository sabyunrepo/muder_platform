<!-- STATUS-START -->
**Active**: Phase 18.1 hotfix — 전체 완료 ✅
**PR**: 5/5 merged (PR-0,1,2,3,4)
**Task**: 41/41 tasks done
**State**: archived
**Blockers**: none
**Last updated**: 2026-04-15
**Final commit**: 407e3d0
<!-- STATUS-END -->
...
- [x] Task 1 — `room.handler.StartRoom`: `http.MaxBytesReader(256KB)` + `ReadJSON` 실패 시 명시 400
- [x] Task 2 — `engine.ParseGameConfig`: `json.Decoder + DisallowUnknownFields` + 최대 modules 수 제한 (e.g., 50)
- [x] Task 3 — `BuildModules`: admin-only 모듈 allowlist 개념 (레지스트리 레벨 tag) — 현재 admin 모듈 없으면 아키텍처 hook 만 마련
- [x] Task 4 — 단위 테스트: oversized body 거부, unknown field 거부, too-many modules 거부
- [x] Task 5 — 기존 정상 flow 회귀 테스트

### PR-3: Frontend store consolidation (B-4)
- [x] Task 1 — `gameSessionStore` 에 `hydrateFromSnapshot` 이관 + `syncServerTime(createdAt)` 호출 추가
- [x] Task 2 — 25 호출자 전수 grep → `gameStore` import 를 `gameSessionStore` 로 치환
- [x] Task 3 — `gameStore.ts` 삭제 (또는 deprecated re-export 로 점진 이관) — 결정: **삭제 후 일괄 치환**
- [x] Task 4 — `gameSelectors.ts` 와 기존 selector 정리 (중복 export 제거)
- [x] Task 5 — Vitest 전체 suite 회귀 없음 확인

**Wave 1 gate**:
- [x] 3 PR 모두 pass
- [x] `go test -race` + `pnpm test` + `tsc --noEmit` pass
- [x] User confirmed next wave
...
- [x] Task 1 — `PLAYWRIGHT_BACKEND=1` 로 localhost(백엔드+Redis+Postgres) 기동 후 `game-session.spec.ts` 실제 실행 통과
- [x] Task 2 — flag off/on 각각의 `StartRoom` 응답 테스트 (503 vs actor start)
- [x] Task 3 — 재접속 E2E 에서 redaction payload 확인 (PR-1 통합)
- [x] Task 4 — CI 에 optional job `phase-18.1-real-backend` 추가 (docker compose 기반, nightly)

**Wave 2 gate**:
- [x] E2E 실행 증적
- [x] User confirmed phase complete

---

## Phase completion gate

- [x] Ship-blocker 4건 + High 4건 전부 처리
- [x] Go race/test + 프론트 test/tsc pass
- [x] Real-backend E2E 1회 pass
- [x] Medium 10건은 Phase 18.2 로 이월 문서화
- [x] `/plan-finish` 실행

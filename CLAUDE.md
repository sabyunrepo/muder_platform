...
| 2026-04-21 | Phase 19 Residual W1+W2 완료 — mockgen + Coverage Gate 도입 | `apps/server/go.mod`(tool directive), `.github/workflows/ci.yml`(coverage-guard enforcement), `codecov.yml`, `memory/project_phase19_residual_progress.md` | 5 PR 머지 (#131~#135). mockgen 신규 도입 (go.uber.org/mock v0.6.0 + Go 1.24 tool directive + 5 패키지 gomock 전환). Coverage Gate warn-only → enforcement (Go 41% / FE Lines 49% / Branches 77% / Functions 53%, baseline-2% buffer 규칙). infra 3 패키지 0% → 43-58% 복구. F-sec-4 admin recordAudit fallback (session-less 컨텍스트 + IDENTITY CHECK 조합 무음 폐기 방지). 다음 세션 W3 PR-8 (Module Cache Isolation) 부터 재개. 상세: `memory/project_phase19_residual_progress.md`. |

...
이 문서는 MMP Codex 운영 표면을 어디에 기록할지 정리한다. 목표는 같은 실수를 여러 파일에 중복해서 고치지 않고, CI 비용을 늘리는 초소형 PR을 줄이며, PR steward가 Codex review check만 보고 완료로 착각하지 않게 만드는 것이다.
...
- 개발 최소 워커 모드에서는 PR 생성 전 `scripts/mmp-local-ci.sh quick`을 기본 검증으로 사용한다. coverage, E2E, Docker 영향이 있으면 같은 스크립트의 `coverage`, `e2e`, `docker`, `full` 모드로 넓힌다.
- GitHub Actions full CI는 매 PR 자동 검증이 아니라 Codex review 정리 후 위험 PR에 붙이는 최종 확인 버튼이다.

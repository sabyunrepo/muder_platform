...
description: Use when creating, reviewing, updating, checking, or merging MMP pull requests, especially when Codex review, focused local validation, or Korean PR descriptions are involved.
---

# MMP PR Lifecycle

## When
- Before opening a PR, resolving Codex review feedback, checking PR status, or merging.
...
3. PR sequence:
   ...
   - inspect Codex review threads
   - fix only valid comments and resolve/address them
   - wait for or request Codex re-review with `@codex review`
   ...
   - merge when Codex review is clear, unresolved review threads are 0, GitHub review decision is non-blocking, and focused local validation evidence is recorded
4. API polling cadence: 30-60 seconds unless user explicitly asks for faster status. Main Codex must not run `scripts/mmp-pr-watch.sh` for repeated waiting. Use one-shot checks (`scripts/mmp-pr-status.sh <PR>`, `gh pr view`) in the main thread, and delegate repeated Codex review waiting to `mmp-ci-steward` through `scripts/mmp-ci-steward-handoff.sh <PR>`.
...
6. Development-minimum PR policy:
   ...
   - The script reports `codex-review-only` under current policy. This means GitHub Actions workers are intentionally not PR gates.
   ...
   - Merge after Codex review is clear, unresolved review threads are 0, light checks pass, and focused local checks for changed scripts/config/code pass. If waiting is needed, hand off to `mmp-ci-steward`; do not keep the main thread in a watcher loop.
...
7. CI steward handoff:
   - Use when Codex review waiting would block starting the next issue and the user has approved agent delegation.
   - Generate a copy-ready handoff with `scripts/mmp-ci-steward-handoff.sh <PR>`.
   - The steward owns only the target PR branch/worktree: Codex review fixes, focused checks, push commits, and strict up-to-date branch refresh through `gh pr update-branch <PR>` only when main Codex explicitly asks or merge-conflict/main-drift evidence requires it.
   ...
   - A steward's final report is valid only if it ran `scripts/mmp-pr-status.sh <PR> --fail-on-blocker --allow-behind` on the final head. Treat a green Codex review check as insufficient when a check exists unless unresolved review threads are 0 and GitHub review decision is non-blocking.
   - Give the steward enough autonomy to finish the PR lifecycle inside its target branch: it may fix valid Codex review findings, run focused local validation, push fixes, and resolve only verified target-PR review threads. Merge authority and merge-batch ordering stay with main Codex.
   - A steward must not treat a pending Codex review/CI state as a final result when an allowed next action exists. Pending means the steward keeps watching or advances to the next gate.
   - Codex review clear + unresolved 0 + light/focused validation is a valid steward `MERGE_READY` state. The steward must not add `ready-for-ci` or dispatch workflows.
   - A steward may report `MERGE_READY` when latest head SHA was checked, unresolved threads are 0, Codex review is clear, light checks are green, focused local validation for changed files is recorded, and the handoff scope says `codex-review-only`.
...
8. Merge batch operation:
   ...
   - For low-conflict PRs where Codex review and focused local validation are clear, main Codex may admin merge a `MERGE_CANDIDATE` to avoid burning worker runs on a pure strict-behind blocker.
...
- Codex review valid feedback is fixed or explicitly rejected with reason.

...
var errorDefinitions = map[string]ErrorDefinition{
	...
	ErrMediaReferenceCleanupFailed: def(ErrMediaReferenceCleanupFailed, "media", "application", http.StatusInternalServerError, SeverityHigh, true, "retry_later", "미디어 연결 해제 중 오류가 발생했습니다. 새로고침 후 다시 시도해주세요."),
	ErrMediaStorageDeleteFailed:    def(ErrMediaStorageDeleteFailed, "media", "infrastructure", http.StatusBadGateway, SeverityHigh, true, "retry_later", "파일 저장소에서 미디어를 삭제하지 못했습니다. 잠시 후 다시 시도해주세요."),
	ErrMediaConfigReferenceInvalid: def(ErrMediaConfigReferenceInvalid, "media", "domain", http.StatusUnprocessableEntity, SeverityMedium, false, "contact_support", "테마 설정에 잘못된 미디어 연결 정보가 있어 자동 정리할 수 없습니다. 관리자에게 문의해주세요."),

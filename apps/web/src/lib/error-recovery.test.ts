...
describe('getRecoveryAction', () => {
  ...
  it('user_action이 없으면 대표 code와 status로 fallback한다', () => {
    ...
    expect(getRecoveryAction(apiError({ code: 'MEDIA_STORAGE_DELETE_FAILED' }))).toBe(
      'retry-later'
    );
    expect(getRecoveryAction(apiError({ code: 'MEDIA_CONFIG_REFERENCE_INVALID' }))).toBe(
      'contact-support'
    );
    expect(getRecoveryAction(apiError({ status: 403, code: 'FORBIDDEN' }))).toBe('request-access');
  ...
  it('429 rate limit은 입력 오류가 아니라 나중에 재시도할 오류로 분류한다', () => {
    expect(getRecoveryAction(apiError({ status: 429, code: 'RATE_LIMITED' }))).toBe('retry-later');
  });
});
...
describe('shouldRetryApiError', () => {
  ...
  it('입력/권한 오류는 retryable이 아니면 재시도하지 않는다', () => {
    expect(shouldRetryApiError(apiError({ status: 422, code: 'VALIDATION_ERROR' }), 0)).toBe(false);

...
import { ClueRuntimeEffectCard, type ClueRuntimeEffectCardHandle } from './ClueRuntimeEffectCard';
...
describe('ClueRuntimeEffectCard', () => {
  it('기존 정보 공개 효과를 제작자 언어로 편집하고 저장한다', () => {
    ...
    const saved = ref.current?.getSaveRequest().writeConfig(configJson) as EditorConfig;
  ...
  it('암호 사용을 켠 뒤 암호가 비어 있으면 저장을 막는다', () => {
    ...
    expect(ref.current?.getSaveRequest()).toMatchObject({ valid: false });

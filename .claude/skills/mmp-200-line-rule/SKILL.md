...
description: MMP v3 파일/함수 크기 티어 가드. Go 500줄/함수 80, TS·TSX 400줄/함수 60·컴포넌트 150, MD 200줄. 구현 전에 한도를 예측하고, 초과 예상 시 분할 설계를 먼저 제시하도록 강제. 리팩터·신규 파일·PR 준비 시 트리거.
---

# mmp-file-size-rule — 유형별 티어 하드 리밋

> 스킬 디렉터리명은 `mmp-200-line-rule` 이지만, 규칙은 유형별 티어로 진화했다.
> MD만 200줄 유지. Go/TS는 아래 표 기준.
...
| `.md` | 200 (분할 기준) | - | index 파일, refs 조합 |

> MD는 "무조건 200줄 이하로 요약"이 아니다. 200줄이 넘으면 **요약 대신 분할**한다.
